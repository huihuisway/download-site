    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1" data-admin-style="text-secondary">管理网站标题、描述、底部信息等基础内容</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-5">
          <div className="card !p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2" data-admin-style="text">
              <Settings className="w-4 h-4" data-admin-style="text-primary" />基本信息
            </h3>
            <div className="space-y-4">
              <div>
                <label htmlFor="site-name" className="block text-xs font-semibold uppercase tracking-wider mb-1.5" data-admin-style="text-secondary">站点名称</label>
                ...
                <p className="text-xs mt-1" data-admin-style="text-muted">显示在页面标题和顶部</p>
              </div>
              <div>
                <label htmlFor="site-description" className="block text-xs font-semibold uppercase tracking-wider mb-1.5" data-admin-style="text-secondary">站点描述</label>
                ...
                <p className="text-xs mt-1" data-admin-style="text-muted">显示在页面标题下方（部分主题使用）</p>
              </div>
            </div>
          </div>

          <div className="card !p-5">
            <h3 className="font-semibold mb-4" data-admin-style="text">底部信息</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" data-admin-style="text-secondary">底部文本</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" data-admin-style="text-secondary">ICP 备案号</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" data-admin-style="text-secondary">公安备案号</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-semibold uppercase tracking-wider" data-admin-style="text-secondary">底部链接</label>
                ...
                {siteInfo.footer_links.length === 0 ? (
                  <p className="text-sm italic py-3" data-admin-style="text-muted">暂无链接，点击"添加"新增</p>
                ) : (
                  <div className="space-y-2">
                    {siteInfo.footer_links.map((link, i) => (
                      <div key={i} className="flex flex-col gap-2 sm:flex-row sm:items-center">
                        ...
                        <button onClick={() => handleRemoveLink(i)} className="btn-ghost !p-2" data-admin-style="text-error"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="lg:col-span-2">
          {showPreview && (
            <div className="sticky top-6">
              <div className="card !p-5">
                <h3 className="text-xs font-semibold uppercase tracking-wider mb-4" data-admin-style="text-muted">实时预览</h3>
                <div className="p-4 mb-3" data-admin-style="border">
                  <p className="text-xs mb-2" data-admin-style="text-muted">页头</p>
                  <h4 className="font-bold text-lg" data-admin-style="text">{siteInfo.site_name || '站点名称'}</h4>
                  {siteInfo.site_description && <p className="text-xs mt-1" data-admin-style="text-muted">{siteInfo.site_description}</p>}
                </div>
                <div className="p-4" data-admin-style="border">
                  <p className="text-xs mb-2" data-admin-style="text-muted">页脚</p>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <p className="text-xs" data-admin-style="text-secondary">{siteInfo.footer_text || '底部文本'}</p>
                    <div className="flex gap-3">
                      {siteInfo.footer_links.map((link, i) => (
                        <span key={i} className="text-xs cursor-pointer" data-admin-style="text-primary">{link.label || '(未命名)'}</span>
                      ))}
                    </div>
                  </div>
                </div>
